/*
 * drivers/cpufreq/cpufreq_terrasilent.c
 *
...
static int cpufreq_governor_terrasilent(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_TERRASILENT
static
#endif
...
static void cpufreq_terrasilent_timer(unsigned long cpu)
{
	...
	delta_time = cputime64_sub(update_time, this_terrasilent->idle_exit_time);
...
static void cpufreq_idle(void)
{
	...
	if (policy->cur == policy->min && timer_pending(&this_terrasilent->timer))
		del_timer(&this_terrasilent->timer);
...
static void cpufreq_terrasilent_freq_change_time_work(struct work_struct *work)
{
	...
	for_each_possible_cpu(cpu) {
		...
		old_freq = this_terrasilent->old_freq;
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_terrasilent->cur_policy = new_policy;
...
static int __init cpufreq_terrasilent_init(void)
{
	unsigned int i;
	struct terrasilent_cpu_info *this_terrasilent;
#ifdef CONFIG_CPU_FREQ_GOV_TERRASILENT_DEBUG
	debug_mask = 1;
#else
	debug_mask = 0;
#endif
	...
	for_each_possible_cpu(i) {
		...
		this_terrasilent->idle_exit_time = 0;
		this_terrasilent->freq_change_time = 0;
		this_terrasilent->freq_change_time_in_idle = 0;
...
fs_initcall(cpufreq_terrasilent_init);
#else
module_init(cpufreq_terrasilent_init);
#endif

static void __exit cpufreq_terrasilent_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_terrasilent);
